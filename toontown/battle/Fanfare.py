...
def makePanel(toon, showToonName):
    panel = DirectFrame(
        relief=None, geom=DGG.getDefaultDialogGeom(),
    ...
    if showToonName is 1:
        panel.avNameLabel = DirectLabel(
            parent=panel, relief=None, pos=Vec3(
                0, 0, 0.3), text=toon.getName(), text_scale=0.08)
    return panel
...
def doFanfare(delay, toon, panel):
    ...
    ballShake1 = Sequence(
        Parallel(
            LerpHprInterval(
                partyBallLeft, duration=0.2,
                startHpr=Vec3(90, 0, 0),
                hpr=Vec3(90, 10, 0),
                blendType='easeInOut'),
            LerpHprInterval(
                partyBallRight, duration=0.2,
                startHpr=Vec3(-90, 0, 0),
                hpr=Vec3(-90, -10, 0),
                blendType='easeInOut')),
        Parallel(
    ...
    trumpturn2 = LerpHprInterval(
        trumpet2, duration=4, startHpr=Vec3(-80, 15, 0),
        hpr=Vec3(-150, 40, 0))
    ...
    trumpetsAppear = LerpColorInterval(
        trumpetNode, 0.3, startColor=Vec4(
            1, 1, 0, 0), color=Vec4(
            1, 1, 0, 1))
    ...
    ballAppear = Parallel(
        LerpColorScaleInterval(
            partyBallLeft, 0.3, startColorScale=Vec4(
                1, 0, 0, 0), colorScale=Vec4(
                1, 0, 0, 1)), LerpColorScaleInterval(
                    partyBallRight, 0.3, startColorScale=Vec4(
    ...
    play = Parallel(
        SoundInterval(
            crabHorn, startTime=1.5, duration=4.0, node=toon),
        Sequence(
            Wait(0.25),
            longshake([trumpet1, trumpet2],
                      3, 0.2),
            Wait(0.5),
            longshake([trumpet1, trumpet2],
                      3, 0.2),
