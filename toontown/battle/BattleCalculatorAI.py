...
class BattleCalculatorAI:
	...
	def __init__(self, battle, tutorialFlag=0):
		...
		self.__skillCreditMultiplier = self.XpMultiplier

	def setSkillCreditMultiplier(self, mult):
		self.__skillCreditMultiplier = self.XpMultiplier * mult
			...
			if atkTrack == LURE:
				if self.getSuitTrapType(targetId) == NO_TRAP:
					...
					if trapCreatorId > 0:
						...
						self.__addAttackExp(
							attack, track=TRAP, level=attackLevel,
				...
				if targetLured and(
						...
						or
						targetId in self.
						successfulLures and self.successfulLures[targetId][1] <
						atkLevel):
					self.notify.debug(
						'Adding target ' + str(targetId) +
				...
				toon = self.battle.getToon(toonId)
				if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[
					...
					if suit:
						...
						if costToFire > abilityToFire:
							simbase.air.writeServerEvent(
								'suspicious',
								avId=toonId,
								issue='Toon attempting to fire a %s cost cog with %s pinkslips' %
								(costToFire,
								 abilityToFire))
				...
				if result > 0 and atkTrack != HEAL and atkTrack != DROP and atkTrack != PETSOS:
					...
					for currInfo in lureInfos:
						if currInfo[3]:
							...
							self.__addAttackExp(
								attack, track=attackTrack, level=currInfo[1],
		...
		for currPrevAtk in range(attackIdx - 1, -1, -1):
			...
			if self.__attackHasHit(prevAttack) and (
				attackAffectsGroup(
					prvAtkTrack,
					prvAtkLevel,
					prevAttack[TOON_TRACK_COL]) or attackAffectsGroup(
					atkTrack,
					atkLevel,
					attack[TOON_TRACK_COL]) or attack[TOON_TGT_COL] == prevAttack[TOON_TGT_COL]) and atkTrack != prvAtkTrack:
				numPrevHits += 1

		if numPrevHits > 0 and self.notify.getDebug():
			self.notify.debug(
				'ACC BONUS: toon attack received accuracy ' + 'bonus of ' +
				str(self.AccuracyBonuses[numPrevHits]) +
				' from previous attack by (' + str(attack[TOON_ID_COL]) +
				') which hit')
	...
	def __addAttackExp(self, attack, track=-1, level=-1, attackerId=-1):
		...
		if trk != -1 and trk != NPCSOS and trk != PETSOS and lvl != -1 and id != -1:
			...
			expList[trk] = min(ExperienceCap, expList[
							   trk] + (lvl + 1) * self.__skillCreditMultiplier)
		return
	...
	def __clearTgtDied(self, tgt, lastAtk, currAtk):
		...
		if currAtkTrack == lastAtkTrack and lastAtk[
				SUIT_DIED_COL] & 1 << position and self.__attackHasHit(currAtk, suit=0):
			if self.notify.getDebug():
				self.notify.debug(
	...
	def __processBonuses(self, hp=1):
		...
		for currTgt in bonusList:
			for currAtkType in currTgt.keys():
				if len(
						currTgt[currAtkType]) > 1 or not hp and len(
	...
	def __clearLuredSuitsByAttack(self, toonId, kbBonusReq=0, targetId=-1):
		...
		if targetId != -1 and self.__suitIsLured(t.getDoId()):
			...
			for t in tgtList:
				if self.__suitIsLured(
					t.getDoId()) and (
					not kbBonusReq or self.__bonusExists(
						t,
						hp=0)):
					self.__removeLured(t.getDoId())
					if self.notify.getDebug():
	...
	def __addLuredSuitsDelayed(
		...
		if targetId != -1:
			...
			for t in tgtList:
				if self.__suitIsLured(
					t.getDoId()) and t.getDoId() not in self.delayedUnlures and (
	...
	def __knockBackAtk(self, attackIndex, toon=1):
		if toon and(
				self.battle.toonAttacks[attackIndex][TOON_TRACK_COL] ==
				THROW or self.battle.toonAttacks[attackIndex][TOON_TRACK_COL] ==
				SQUIRT):
			if self.notify.getDebug():
	...
	def __calcSuitTarget(self, attackIndex):
		...
		if suitId in self.SuitAttackers and random.randint(0, 99) < 75:
			...
			dmgs = []
			for currToon in self.SuitAttackers[suitId].keys():
				dmgs.append(self.SuitAttackers[suitId][
							currToon] / totalDamage * 100)
		...
		if self.APPLY_HEALTH_ADJUSTMENTS:
			for t in self.battle.activeToons:
				...
				if toonHp - attack[SUIT_HP_COL][position] <= 0:
					...
					attack[TOON_DIED_COL] = attack[
						TOON_DIED_COL] | 1 << position
				if self.notify.getDebug():
					self.notify.debug(
						'Toon ' + str(t) + ' takes ' +
						str(attack[SUIT_HP_COL][position]) + ' damage')
	...
	def __printSuitAtkStats(self):
		...
		for currTgt in self.suitAtkStats.keys():
			...
			self.notify.debug(
				' toon ' + str(currTgt) + ' at position ' + str(tgtPos) +
				' was attacked ' + str(self.suitAtkStats[currTgt]) + ' times')
	...
	def __calculateSuitAttacks(self):
		for i in range(len(self.battle.suitAttacks)):
			if i < len(self.battle.activeSuits):
				...
				if self.notify.getDebug():
					self.notify.debug(
						'Suit attack: ' + str(self.battle.suitAttacks[i]))
	...
	def __updateActiveToons(self):
		...
		for trap in self.traps.keys():
			if self.traps[trap][1] not in self.battle.activeToons:
				self.notify.debug(
					'Trap for toon ' + str(self.traps[trap][1]) +
	...
	def __luredMaxRoundsReached(self, suitId):
		return self.__suitIsLured(suitId) and self.currentlyLuredSuits[suitId][
			0] >= self.currentlyLuredSuits[suitId][1]

	def __luredWakeupTime(self, suitId):
		return self.__suitIsLured(suitId) and self.currentlyLuredSuits[suitId][
			0] > 0 and random.randint(0, 99) < self.currentlyLuredSuits[suitId][2]
