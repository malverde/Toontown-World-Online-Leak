...
class DistributedBattleBaseAI(
    ...
    def __init__(
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            'DistributedBattleAI', [
                State.State(
                    'FaceOff', self.enterFaceOff, self.exitFaceOff, [
                                    ...
                                    'MakeMovie', self.enterMakeMovie, self.exitMakeMovie, [
                                        'PlayMovie', 'Resume']), State.State(
                                            'PlayMovie', self.enterPlayMovie, self.exitPlayMovie, [
                                                'WaitForJoin', 'Reward', 'Resume']), State.State(
                                                    'Reward', self.enterReward, self.exitReward, ['Resume']), State.State(
                                                        'Resume', self.enterResume, self.exitResume, []), State.State(
                                                            'Off', self.enterOff, self.exitOff, [
                                                                'FaceOff', 'WaitForJoin'])], 'Off', 'Off')
        self.joinableFsm = ClassicFSM.ClassicFSM(
            'Joinable',
            [State.State(
                'Joinable', self.enterJoinable, self.exitJoinable,
                ['Unjoinable']),
             State.State(
                 'Unjoinable', self.enterUnjoinable, self.exitUnjoinable,
                 ['Joinable'])],
        ...
        self.runableFsm = ClassicFSM.ClassicFSM(
            'Runable',
            [State.State(
                'Runable', self.enterRunable, self.exitRunable,
                ['Unrunable']),
             State.State(
                 'Unrunable', self.enterUnrunable, self.exitUnrunable,
                 ['Runable'])],
            'Unrunable', 'Unrunable')
    ...
    def enterWaitForInput(self):
        ...
        if not self.tutorialFlag:
            self.timer.startCallback(
                SERVER_INPUT_TIMEOUT, self.__serverTimedOut)
        self.npcAttacks = {}
    ...
    def enterPlayMovie(self):
        ...
        self.runableFsm.request('Unrunable')
        self.resetResponses()
        movieTime = TOON_ATTACK_TIME * (
            len(self.activeToons) + self.numNPCAttacks) + SUIT_ATTACK_TIME * len(
    ...
    def __movieDone(self):
        ...
        for activeToon in self.activeToons + self.exitedToons:
            if activeToon in self.toonAttacks:
                ...
                npc_level = None
                if track == NPCSOS:
                    track, npc_level, npc_hp = NPCToons.getNPCTrackLevelHp(attack[
                                                                           TOON_TGT_COL])
                    if track is None:
                        track = NPCSOS
                    elif track == TRAP:
                        ...
                        if toon is not None:
                            ...
                            if check == -1:
                                self.air.writeServerEvent(
                                    'suspicious',
                                    avId=toonId,
                                    issue='Toon generating movie for non-existant gag track %s level %s' %
                                    (track,
                                     level))
                                self.notify.warning(
            ...
            if attack != NO_ATTACK:
                ...
                if not (hasattr(suit, 'dna') and suit.dna):
                    ...
                    self.notify.warning(
                        ...
                        toonId)
                    self.air.writeServerEvent(
                        'suspicious', avId=toonId,
                        issue='_movieDone avoiding crash, suit has no dna')
                    continue
                adict = getSuitAttack(
    ...
    def enterResume(self):
        ...
        self.air.writeServerEvent(
            'battle-cogs-defeated',
            battleId=self.doId,
            avIds=self.toons,
            taskZoneId=self.getTaskZoneId(),
            suits=suitsLog)
...
# VERIFICATION FAILED - in __init__ LOAD_GLOBAL 40(None) needs to be
# changed to a LOAD_CONST
