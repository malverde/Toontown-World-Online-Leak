...
def doSquirts(squirts):
    ...
    for squirt in squirts:
        ...
        if skip:
            pass
        elif isinstance(squirt['target'], type([])):
            if 1:
...
def __doSeltzerBottle(squirt, delay, fShowStun):
    ...
    bottleTrack = Sequence(
        ...
        LerpScaleInterval(
            bottle, dBottleScale, bottle.getScale(),
            startScale=MovieUtil.PNT3_NEARZERO),
...
def __doFireHose(squirt, delay, fShowStun):
    ...
    propTrack = Sequence(
        ...
        LerpScaleInterval(
            hydrantScale, tAppearDelay * 0.5, Point3(1, 1, 1.4),
            startScale=Point3(1, 1, 0.01)),
        LerpScaleInterval(
            hydrantScale, tAppearDelay * 0.3, Point3(1, 1, 0.8),
            startScale=Point3(1, 1, 1.4)),
        LerpScaleInterval(
            hydrantScale, tAppearDelay * 0.1, Point3(1, 1, 1.2),
            startScale=Point3(1, 1, 0.8)),
        LerpScaleInterval(
            hydrantScale, tAppearDelay * 0.1, Point3(1, 1, 1),
            startScale=Point3(1, 1, 1.2)),
        ActorInterval(hose, 'firehose', duration=dAnimHold),
        Wait(dHoseHold - 0.2),
        LerpScaleInterval(
            hydrantScale, 0.2, Point3(1, 1, 0.01),
    ...
    # The stream from the fire hose needs to be scaled with the cog's
    # animation - right now it is static.
    if hp > 0 or delay <= 0:
        tracks.append(
            __getSuitTrack(
                suit,
