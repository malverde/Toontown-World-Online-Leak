...
class CatalogItemPanel(DirectFrame):
    ...
    def load(self):
        ...
        if self['item'].needsCustomize() and(
                typeCode == CatalogItemTypes.WALLPAPER_ITEM or typeCode ==
                CatalogItemTypes.FLOORING_ITEM or typeCode == CatalogItemTypes.
                MOULDING_ITEM or typeCode == CatalogItemTypes.
                FURNITURE_ITEM or typeCode == CatalogItemTypes.
                WAINSCOTING_ITEM or typeCode == CatalogItemTypes.TOON_STATUE_ITEM):
        ...
        self.typeLabel = DirectLabel(
            parent=self, relief=None, pos=(0, 0, 0.24),
            scale=TTLocalizer.CIPtypeLabel, text=self['item'].getTypeName(),
        ...
        self.priceLabel = DirectLabel(
            parent=self, relief=None, pos=(0, 0, -0.3),
            scale=priceScale, text=priceStr, text_fg=(0.95, 0.95, 0, 1),
            text_shadow=(0, 0, 0, 1),
        ...
        self.buyButton = DirectButton(
            parent=self, relief=None, pos=(0.2, 0, 0.15),
            scale=(0.7, 1, 0.8),
        ...
        self.soundOnButton = DirectButton(
            parent=self, relief=None, pos=(0.2, 0, -0.15),
            scale=(0.7, 1, 0.8),
            text_scale=buyTextScale, text_pos=(-0.005, -0.01),
            image=(upButton, downButton, rolloverButton, upButton),
        ...
        if emblemPrices:
            if numericBeanPrice:
                ...
                self.beanPriceLabel = DirectLabel(
                    parent=self, relief=None, pos=(0, 0, -0.3),
                    scale=priceScale, image=beanModel,
                    image_pos=(-0.4, 0, 0.4),
            ...
            if ToontownGlobals.EmblemTypes.Gold < len(emblemPrices):
                ...
                if goldPrice:
                    self.goldPriceLabel = DirectLabel(
                        parent=self, relief=None, pos=(0, 0, -0.3),
                        scale=priceScale, image=goldModel,
                        image_pos=(-0.4, 0, 0.4),
                        text=str(goldPrice),
                        text_fg=(0.95, 0.95, 0, 1),
                        text_shadow=(0, 0, 0, 1),
    ...
    def updateBuyButton(self):
        ...
        if isNameTag and not localAvatar.getGameAccess() == OTPGlobals.AccessFull:
            ...
            auxText = TTLocalizer.DaysToGo % self[
                'item'].getDaysToGo(base.localAvatar)
            self.buyButton['state'] = DGG.DISABLED
    ...
    def __handleGiftRequest(self):
        if self['item'].replacesExisting() and self['item'].hasExisting():
            ...
            message = TTLocalizer.CatalogVerifyGift % {
                ...
                'friend': friendText}
        self.verify = TTDialog.TTGlobalDialog(
            doneEvent='verifyGiftDone', message=message,
            style=TTDialog.TwoChoice)
        self.verify.show()
        ...
        if numFriends > 0:
            ...
            if self.parentCatalogScreen.gotAvatar == 1:
                ...
                if len(
                        ...
                        avatar.onGiftOrder) >= ToontownGlobals.MaxMailboxContents:
                    self.giftButton.show()
                    self.giftButton['state'] = DGG.DISABLED
                    auxText = TTLocalizer.CatalogMailboxFull
                    self.auxText['text'] = auxText
                    return
                if self['item'].getPrice(
