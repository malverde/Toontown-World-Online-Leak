...
class DistributedStartingBlockAI(DistributedObjectAI):
    ...
    def requestExit(self):
        ...
        if avId != self.avId:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to get off a starting block they\'re not on!')
    ...
    def movieFinished(self):
        ...
        if self.avId != avId:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to end movie of another toon!')
            return
        if not self.currentMovie:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to end non-existent movie!')
            return
...
class DistributedViewingBlockAI(DistributedStartingBlockAI):
    ...
    def requestEnter(self, isPaid):
        ...
        taskMgr.doMethodLater(
            30, DistributedViewingBlockAI.b_setMovie, 'removePlayer%i' %
            self.doId, [
                self, KartGlobals.EXIT_MOVIE])
