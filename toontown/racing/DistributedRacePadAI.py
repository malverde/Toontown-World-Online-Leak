...
class DistributedRacePadAI(DistributedKartPadAI, FSM):
    ...
    def enterWaitEmpty(self):
        ...
        taskMgr.doMethodLater(
            30,
            DistributedRacePadAI.changeTrack,
            'changeTrack%i' %
            self.doId,
            [self])
    ...
    def enterWaitCountdown(self):
        taskMgr.doMethodLater(
            30,
            DistributedRacePadAI.startRace,
            'startRace%i' %
            self.doId,
            [self])
    ...
    def enterAllAboard(self):
        taskMgr.doMethodLater(
            2,
            DistributedRacePadAI.createRace,
            'createRace%i' %
            self.doId,
            [self])
    ...
    def changeTrack(self):
        nri = RaceGlobals.getNextRaceInfo(
            self.trackId, self.nameType, self.index)
            ...
            if avId in self.air.doId2do:
                ...
                if av.getTickets() < entryFee:
                    self.air.writeServerEvent(
                        ...
                        issue='Toon somehow lost tickets between entering a race and it leaving!')
                    av.b_setTickets(0)
                else:
