...
class DistributedRace(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            'Race', [
                State.State(
                    'join', self.enterJoin, self.exitJoin, [
                        'prep', 'leave']), State.State(
                    'prep', self.enterPrep, self.exitPrep, [
                        'tutorial', 'leave']), State.State(
                            'tutorial', self.enterTutorial, self.exitTutorial, [
                                'start', 'waiting', 'leave']), State.State(
                                    'waiting', self.enterWaiting, self.exitWaiting, [
                                        'start', 'leave']), State.State(
                                            'start', self.enterStart, self.exitStart, [
                                                'racing', 'leave']), State.State(
                                                    'racing', self.enterRacing, self.exitRacing, [
                                                        'finished', 'leave']), State.State(
                                                            'finished', self.enterFinished, self.exitFinished, ['leave']), State.State(
                                                                'leave', self.enterLeave, self.exitLeave, [])], 'join', 'leave')
    ...
    def enterTutorial(self):
        ...
        self.rulesPanel = MinigameRulesPanel.MinigameRulesPanel(
            'RacingRulesPanel', self.getTitle(),
    ...
    def raceWatcher(self, task):
        ...
        if nextPole < self.currentPole:
            newT = self.curveTs[
                self.currentPole] * (1 - t) + self.curve.getMaxT() * t
        else:
            newT = self.curveTs[
                self.currentPole] * (1 - t) + self.curveTs[nextPole] * t
    ...
    def setupGeom(self):
        ...
        for i in range(4000):
            ...
            self.curve.getPoint(
                i / 4000.0 * (self.curve.getMaxT() - 1e-11),
                self.curvePoints[-1])
    ...
    def getNearestT(self, pos):
        ...
        if nextPole < pole:
            t = self.curveTs[
                pole] * (1 - percent) + self.curve.getMaxT() * percent
        else:
            t = self.curveTs[
                pole] * (1 - percent) + self.curveTs[nextPole] * percent
        return t
