...
class DistributedRaceAI(DistributedObjectAI, FSM):
    ...
    def generate(self):
        for avatar in self.avatars:
            self.acceptOnce(
                self.air.getAvatarExitEvent(avatar),
                self.playerLeave, [avatar])
        self.demand('Join')
    ...
    def enterStart(self):
        self.startTime = globalClockDelta.networkToLocalTime(
            globalClockDelta.getRealNetworkTime()) + 3
        self.b_startRace(3)
    ...
    def d_startRace(self, timeUntilStart):
        self.sendUpdate('startRace', [globalClockDelta.localToNetworkTime(
            globalClockDelta.globalClock.getRealTime() + timeUntilStart)])
    ...
    def hasGag(self, slot, requestedGag, index):
        ...
        if avId not in self.avatars:
            self.air.writeServerEvent(
                ...
                issue='Toon tried to get gag in a race they\'re not in!')
            return
        if self.raceType == RaceGlobals.Practice:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to gain gag in a practice race!')
            return
        ...
        if not gag[0]:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to pick up a gag that doesn\'t exist!')
            return
        gagIndex = gag[1]
        ...
        if realGag != requestedGag:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to get the wrong gag!')
            return
        self.gags[slot] = [0, 0]
        self.avatarGags[avId] = requestedGag
        taskMgr.doMethodLater(
            5, self.__regenGag, 'regenGag%i-%i' %
            (slot, self.doId), [slot])
    ...
    def racerLeft(self, avId):
        ...
        if realAvId != avId:
            self.air.writeServerEvent(
                'suspicious', avId=realAvId,
                issue='Toon tried to make another quit race!')
            return
        if avId not in self.avatars:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to leave race they\'re not in!')
            return
        self.avatars.remove(avId)
        if set(
            self.finishedAvatars) == set(
            self.avatars) or len(
                self.avatars) == 0:
            self.requestDelete()
    ...
    def heresMyT(self, avId, laps, currentLapT, timestamp):
        ...
        if not avId == realAvId:
            self.air.writeServerEvent(
                'suspicious', avId=realAvId,
                issue='Toon tried to send a message as another toon!')
            return
        if avId not in self.avatars:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon not in race tried to send update to race!')
            return
    ...
    def avatarFinished(self, avId):
        if avId not in self.avatars:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to finish in a race they\'re not in!')
            return

        if avId in self.finishedAvatars:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to finish in a race twice!')
            return
        ...
        if self.raceType == RaceGlobals.Practice:
            ...
            trophies = self.calculateTrophies(
                avId, place == 1, qualify, totalTime)
        else:
            winnings = 0
            trophies = []
    ...
    def requestThrow(self, x, y, z):
        ...
        if self.avatarGags[avId] == RaceGlobals.BANANA:
            ...
            places = sorted(
                ...
                reverse=True)
            for i in places:
                if not i in self.finishedAvatars and not self.currentlyAffectedByAnvil[
                        i]:
                    currAvatar = i
                    break
            self.currentlyAffectedByAnvil[avId] = True
            taskMgr.doMethodLater(
                RaceGlobals.AnvilSquishDuration,
                self.unsquish,
                'unsquish-%i' %
                currAvatar,
                [currAvatar])
            self.sendUpdate(
                'dropAnvilOn', [
                    avId, currAvatar, globalClockDelta.getRealNetworkTime()])
    ...
    def requestKart(self):
        ...
        if avId not in self.avatars:
            self.air.writeServerEvent(
                'suspicious', avId=avId,
                issue='Toon tried to request kart in race they\'re not in!')
            return
        for aK in self.avatarKarts:
            if aK[0] == avId:
