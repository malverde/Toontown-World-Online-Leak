    ...
    class MainMenuDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            DirectFrame.__init__(
                self, relief=None, state='normal', geom=model, text_scale=0.1,
                geom_scale=self.modelScale, pos=(0, 0, -.01),
                frameSize=(-1, 1, -1, 1))
            self.initialiseoptions(KartShopGuiMgr.MainMenuDlg)
            self.cancelButton = DirectButton(
                parent=self, relief=None, geom=model.find('**/CancelIcon'),
                image=(model.find('**/CancelButtonUp'),
                       model.find('**/CancelButtonDown'),
                       model.find('**/CancelButtonRollover')),
                scale=self.modelScale, pressEffect=False,
                command=lambda: messenger.send(
                    doneEvent, [MM_OPTIONS.Cancel]))
            self.buyKartButton = DirectButton(
                ...
                scale=self.modelScale, geom=model.find('**/BuyKartIcon'),
                text=TTLocalizer.KartShop_BuyKart, text_scale=KS_TEXT_SIZE_BIG,
                text_pos=(-0.2, 0.34),
                pressEffect=False, command=lambda: messenger.send(
                    doneEvent, [MM_OPTIONS.BuyKart]))
            self.buyAccessoryButton = DirectButton(
                ...
                scale=self.modelScale, text=TTLocalizer.
                KartShop_BuyAccessories, text_scale=KS_TEXT_SIZE_BIG,
                text_pos=(-0.1, 0.036),
                pressEffect=False, command=lambda: messenger.send(
                    doneEvent, [MM_OPTIONS.BuyAccessory]))
            self.updateButtons()
            return
        ...
        def __init__(self, doneEvent):
            ...
            self.buyKartButton = DirectButton(
                ...
                image=(model.find('**/BuyKartButtonUp'),
                       ...
                       model.find('**/BuyKartButtonRollover'),
                       model.find('**/BuyKartButtonDisabled')),
                scale=self.modelScale, text=TTLocalizer.KartShop_BuyKart,
                text_scale=KS_TEXT_SIZE_BIG, text_pos=(0, -.534),
                pressEffect=False, command=lambda: messenger.send(
            ...
            self.arrowLeftButton = DirectButton(
                ...
                scale=self.modelScale, pressEffect=False,
                command=self.__handleKartChange, extraArgs=[-1])
            ...
            self.kartView.load(
                ...
                ['rotate_right_up', 'rotate_right_down', 'rotate_right_roll',
                 'rotate_right_down', (0.255, -.054)],
                ['rotate_left_up', 'rotate_left_down', 'rotate_left_roll',
                 'rotate_left_down', (-.24, -.054)],
                (0, -.055))
    ...
    class ReturnKartDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.okButton = DirectButton(
                ...
                scale=self.modelScale, pressEffect=False,
                command=lambda: messenger.send(
                    doneEvent, [RK_OPTIONS.ReturnKart]))
    ...
    class ConfirmBuyKartDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, kartNum):
            ...
            self.okButton = DirectButton(
                ...
                scale=self.modelScale, pressEffect=False,
                command=lambda: messenger.send(
                    doneEvent, [CBK_OPTIONS.BuyKart]))
            self.kartView = KartViewer(
                [self.kartNum, -1, -1, -1, -1, -1, -1, -1, -1],
                parent=self)
            self.kartView.setPos(model.find('**/KartViewerFrame').getPos())
            self.kartView.load(model, 'KartViewerFrame', [], [], None)
            self.kartView.setBounds(-0.38, 0.38, -.0425, 0.49)
    ...
    class BuyAccessoryDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            DirectFrame.__init__(
                ...
                geom_scale=self.modelScale, frameSize=(-1, 1, -1, 1),
                pos=(0, 0, -0.01),
                text_wordwrap=26, text_scale=0.1,
            ...
            self.arrowLeftButton = DirectButton(
                ...
                image=(model.find('**/ArrowLeftButtonUp'),
                       ...
                       model.find('**/ArrowLeftButtonRollover'),
                       model.find('**/ArrowLeftButtonInactive')),
                scale=self.modelScale, text_pos=(0, 0),
                text_scale=0.1, pressEffect=False,
                command=self.__handleAccessoryChange, extraArgs=[-1])
            self.arrowRightButton = DirectButton(
                parent=self,
                relief=None,
                image=(
            ...
            self.cancelButton = DirectButton(
                ...
                image=(model.find('**/CancelButtonUp'),
                       ...
                       model.find('**/CancelButtonRollover')),
                geom=model.find('**/CancelIcon'),
                scale=self.modelScale, command=lambda: messenger.send(
                    doneEvent, [BA_OPTIONS.Cancel]),
                pressEffect=False)
            ...
            self.buyAccessoryButton = DirectButton(
                ...
                image=(model.find('**/BuyAccessoryButtonUp'),
                       ...
                       model.find('**/BuyAccessoryButtonRollover'),
                       model.find('**/BuyAccessoryButtonDisabled')),
                scale=self.modelScale, text=TTLocalizer.KartShop_BuyAccessory,
                text_pos=(0, -.57),
                text_scale=KS_TEXT_SIZE_SMALL, pressEffect=False,
            ...
            self.kartView = KartViewer(
                list(base.localAvatar.getKartDNA()),
                parent=self)
            self.kartView.setPos(model.find('**/KartViewerFrame').getPos())
            self.kartView.load(
                model, 'KartViewerFrame',
        ...
        def __handleBuyAccessory(self):
            accessoryID = self.unownedAccDict[
                self.curAccType][
        ...
        def __handleAccessoryChange(self, nDir):
            if len(self.unownedAccDict[self.curAccType]) < 1:
                ...
                self.curAccIndex[self.curAccType] = (self.curAccIndex[
                                                     self.curAccType] + nDir) % len(self.unownedAccDict[self.curAccType])
        ...
        def showAccessory(self):
            ...
            if len(self.unownedAccDict[self.curAccType]) < 1:
                ...
                if self.curAccIndex[
                        self.curAccType] + 1 < len(self.unownedAccDict[self.curAccType]):
                    self.arrowRightButton['state'] = DGG.NORMAL
                ...
                cost = TTLocalizer.KartShop_Cost % AccessoryDict[
                    curAcc][KartInfo.cost]
            ...
            if len(base.localAvatar.getKartAccessoriesOwned()
                ...
                self.buyAccessoryButton.configure(
                    text_scale=KS_TEXT_SIZE_SMALL * 0.8)
                self.buyAccessoryButton.configure(
        ...
        def __init__(self, doneEvent, accID):
            ...
            self.ticketDisplay = DirectLabel(
                parent=self, relief=None, text=str(
                    ...
                    0.95, 0.95, 0.0, 1.0), text_shadow=(
                    0, 0, 0, 1), text_pos=(
                    0.43, -0.5), text_font=ToontownGlobals.getSignFont())
            self.okButton = DirectButton(
                parent=self, relief=None,
                image=(model.find('**/CheckButtonUp'),
                       model.find('**/CheckButtonDown'),
    ...
    class ConfirmBuyAccessoryDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, accID):
            ...
            DirectFrame.__init__(
                ...
                pos=(0, 0, -0.01),
                text=TTLocalizer.KartShop_ConfirmBuy % (name, cost),
                text_wordwrap=14, text_scale=KS_TEXT_SIZE_SMALL,
                text_pos=(0, -0.25))
            ...
            self.okButton = DirectButton(
                ...
                scale=self.modelScale, pressEffect=False,
                command=lambda: messenger.send(
                    doneEvent, [CBA_OPTIONS.BuyAccessory]))
            self.kartView = DirectFrame(
                parent=self, relief=None,
                geom=model.find('**/KartViewerFrame'),
                scale=1.0)
