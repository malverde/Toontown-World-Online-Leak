...
class NewsManager(DistributedObject.DistributedObject):
	...
	def setInvasionStatus(self, msgType, cogType, numRemaining, specialSuit):
		...
		if specialSuit == 1:
			...
			cogNameP = TTLocalizer.SkeleReviveCogName % {
				'cog_name': SuitBattleGlobals.SuitAttributes[cogType]['pluralname']}
		else:
	...
	def startDecoTick():
		...
		taskMgr.doMethodLater(
			nextHour,
			setWinterDecorationsStart,
			'hourly-DECO')
	def setWinterDecorationsStart():
		task.delayTime = 3600
		base.localAvatar.setSystemMessage(
			...
			while i < len(item):
				...
				while True:
					...
					nDay = self.dayForWeekday(
						theDate.year, item[i + 1][0],
						eWeekday, eRepNum)

					if ((nDay > sDay and item[i +
											  1][0] == item[i][0] and (item[i +
																			1][1] -
																	   item[i][1]) <= (nDay -
																					   sDay +
																					   abs(eWeekday -
