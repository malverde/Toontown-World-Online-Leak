    ...
    for fishGenus in TTLocalizer.FishSpeciesNames:
        ...
        for speciesName in fishGenusSpeciesList:
            if fishName.lower() == speciesName.lower():
                simbase.air.fishManager.requestedFish[
                    spellbook.getTarget().doId] = fishGenus, fishGenusSpeciesList.index(speciesName)
                return "Request for the fish %s was saved for the avatar %s" % (
                    speciesName, spellbook.getTarget().getName())
