...
class SuitPlannerBase:
    ...
    def performPathTest(self):
        ...
        for i in range(numPathPoints - 1):
            ...
            travelTime = self.dnaStore.suitGraph.getSuitEdgeTravelTime(
                path.getPointIndex(i), path.getPointIndex(i + 1), self.suitWalkSpeed)
            self.notify.debug(
                'edge from point ' + repr(i) + ' to point ' + repr((i + 1)) +
                ' is in zone: ' + repr(zone) + ' and will take ' +
                repr(travelTime) + ' seconds to walk.')

        return None

    def genPath(self, startPoint, endPoint, minPathLen, maxPathLen):
        return self.dnaData.suitGraph.getSuitPath(
