...
class DistributedLawbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def __makeRollToBattleTwoMovie(self):
        ...
        myInterval = base.camera.posHprInterval(
            8, Point3(-22, -100, 35),
            Point3(-10, -13, 0),
        ...
        chatTrack = Sequence(
            Func(
                ...
                base.camera.reparentTo, localAvatar), Func(
                base.camera.setPos, localAvatar.cameraPositions[0][0]), Func(
                    base.camera.setHpr, 0, 0, 0), Func(
                        self.releaseToons, 1))
        ...
        finalReflectedPodiumPos = Point3(
            ...
            ToontownGlobals.LawbotBossBattleTwoPosHpr[2])
        return Sequence(
            chatTrack, bossTrack, Func(self.getGeomNode().setH, 0),
            Parallel(
                self.podium.posInterval(5.0, finalPodiumPos),
                self.reflectedPodium.posInterval(
                    5.0, finalReflectedPodiumPos),
                Func(self.stashBoss),
                self.posInterval(5.0, battlePos),
                Func(
                    taskMgr.doMethodLater, 0.01, self.unstashBoss,
                    'unstashBoss')),
    ...
    def __makeRollToBattleThreeMovie(self):
        ...
        track, hpr = self.rollBossToPoint(
            battlePos, hpr, battlePos, battleHpr, 0)
        self.makeToonsWait()
        return Sequence(chatTrack, bossTrack, Func(
    ...
    def makeEndOfBattleMovie(self, hasLocalToon):
        ...
        if hasLocalToon:
            seq += [Func(self.show),
                    ...
                    Func(
                base.camera.setPos, localAvatar.cameraPositions[0]
                [0]),
    ...
    def loadScale(self):
        self.useProgrammerScale = config.GetBool(
    ...
    def __showCannonsAppearing(self, elapsedTime=0):
        allCannonsAppear = Sequence(
            Func(
                self.__positionToonsInFrontOfCannons), Func(
                base.camera.reparentTo, localAvatar), Func(
                base.camera.setPos, localAvatar.cameraPositions[2][0]), Func(
                    base.camera.lookAt, localAvatar))
    ...
    def enterRollToBattleThree(self):
        ...
        seq = Sequence(self.__makeRollToBattleThreeMovie(), Func(
            self.__onToPrepareBattleThree), name=intervalName)
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(self.betweenBattleMusic, looping=1, volume=0.9)
        ...
        for i in range(len(toonIds)):
            ...
            if toon:
                ...
                ival.append(
                    Sequence(
                        Func(
                            toon.setPlayRate, 0.8, 'walk'), Func(
    ...
    def __makePrepareBattleTwoMovie(self):
        ...
        movie = Sequence(
            Func(
                base.camera.reparentTo, self.witnessToon), Func(
                base.camera.setPos, 0, 8, 2), Func(
                base.camera.setHpr, 180, 10, 0), Func(
    ...
    def __makePrepareBattleThreeMovie(self):
        movie = Sequence(
            Func(base.camera.reparentTo, render),
            Func(base.camera.setPos, -15, 15, 20),
            Func(base.camera.setHpr, -90, 0, 0),
            Wait(3),
