...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    def __init__(self, air, zoneId):
        ...
        if config.GetBool('want-cogdominiums', False):
            if not hasattr(self.__class__, 'CogdoPopAdjusted'):
                ...
                for index in xrange(len(self.SuitHoodInfo)):
                    ...
                    hoodInfo[
                        self.SUIT_HOOD_INFO_BMIN] = int(
                        0.5 +
                        self.CogdoPopFactor *
                        hoodInfo[
                            self.SUIT_HOOD_INFO_BMIN])
                    hoodInfo[
                        self.SUIT_HOOD_INFO_BMAX] = int(
                        0.5 +
                        self.CogdoPopFactor *
                        hoodInfo[
        ...
        self.targetNumSuitBuildings = self.SuitHoodInfo[
            self.hoodInfoIdx][self.SUIT_HOOD_INFO_BMIN]
    ...
    def getZoneIdToPointMap(self):
        ...
        for point in self.streetPointList:
            ...
            for p in points:
                zoneId = self.dnaData.suitGraph.getEdgeZone(
                    self.dnaData.suitGraph.getConnectingEdge(point, p))
    ...
    def createNewSuit(
        ...
        while startPoint is None and len(blockNumbers) > 0:
            ...
            if bn in self.buildingSideDoors:
                for doorPoint in self.buildingSideDoors[bn]:
                    points = self.dnaData.suitGraph.getAdjacentPoints(
                        doorPoint)
                    for p in points:
                        startTime = SuitTimings.fromSuitBuilding
                        startTime += self.dnaData.suitGraph.getSuitEdgeTravelTime(
                            doorPoint, p, self.suitWalkSpeed)
        ...
        gotDestination = self.chooseDestination(
            newSuit, startTime, toonBlockTakeover=toonBlockTakeover,
        ...
        # Determine if we are spawning a special type of suit. 1 is Skelecog, 2
        # is v2.0.
        if specialSuit == 1:
    ...
    def pathCollision(self, path, elapsedTime):
        ...
        for p in path:
            if not (p.getPointType() == DNAStoreSuitPoint.FRONTDOORPOINT or p.getPointType(
            ) == DNAStoreSuitPoint.SIDEDOORPOINT):
                break
            adjacentPoint = path[path.index(p) + 1]
            point = p
            elapsedTime += self.dnaData.suitGraph.getSuitEdgeTravelTime(
                p, adjacentPoint, self.suitWalkSpeed)
    ...
    def battleCollision(self, point, adjacentPoint):
        zoneId = self.dnaData.suitGraph.getEdgeZone(
            self.dnaData.suitGraph.getConnectingEdge(
                point, adjacentPoint))
        ...
        while numToAssign > 0:
            ...
            while repeat and buildingTrack is not None and buildingHeight is not None:
                ...
                if numTarget >= currHoodInfo[
                        self.SUIT_HOOD_INFO_BMAX] or numTarget >= numTotalBuildings:
    ...
    def chooseStreetWithPreference(
        ...
        for currHoodInfo in hoodInfo:
            weight = currHoodInfo[self.SUIT_HOOD_INFO_BWEIGHT]
            thisValue = weight * currHoodInfo[self.SUIT_HOOD_INFO_TRACK][
                buildingTrackIndex] * currHoodInfo[self.SUIT_HOOD_INFO_HEIGHTS][buildingHeight]
    ...
    def requestBattle(self, zoneId, suit, toonId):
        ...
        if config.GetBool(
                'props-buff-battles',
                False) and canonicalZoneId in self.cellToGagBonusDict:
    ...
    def buildingListQuery(self):
        ...
        for dept in SuitDNA.suitDepts:
            if dept in buildingDict:
                buildingList[SuitDNA.suitDepts.index(dept)] = buildingDict[
                    dept]
