...
class Suit(Avatar.Avatar):
    ...
    def generateSuit(self):
        ...
        if dna.name in SuitGlobals.suitProperties:
            self.scale = SuitGlobals.suitProperties[
                dna.name][
                SuitGlobals.SCALE_INDEX]
            self.handColor = SuitGlobals.suitProperties[
                dna.name][SuitGlobals.HAND_COLOR_INDEX]
            ...
            if SuitGlobals.suitProperties[
                    dna.name][
                    SuitGlobals.HEAD_TEXTURE_INDEX]:
                self.headTexture = SuitGlobals.suitProperties[
                    dna.name][SuitGlobals.HEAD_TEXTURE_INDEX]

            for head in SuitGlobals.suitProperties[
                    dna.name][SuitGlobals.HEADS_INDEX]:
                self.generateHead(head)

            self.setHeight(
                SuitGlobals.suitProperties[
                    dna.name][
                    SuitGlobals.HEIGHT_INDEX])
    ...
    def generateAnimDict(self):
        ...
        for anim in AllSuits:
            animDict[
                anim[0]] = 'phase_' + str(bodyPhase) + filePrefix + anim[1]
        ...
        for i in range(0, headReferences.getNumPaths()):
            if config.GetBool('want-new-cogs', 0):
                headPart = self.instance(
                    headReferences.getPath(i),
                    'modelRoot', 'to_head')
                if not headPart:
                    headPart = self.instance(
                        headReferences.getPath(i),
                        'modelRoot', 'joint_head')
            else:
                headPart = self.instance(
                    headReferences.getPath(i),
                    'modelRoot', 'joint_head')
